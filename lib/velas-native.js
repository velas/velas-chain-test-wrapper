...
const default_ERC_20_abi_json_1 = __importDefault(require("./contracts/default-ERC-20-abi.json"));
class AccountObj {
    seed;
    account;
    bufferedSeed;
...
class VelasNative {
     ...
     * in other case some transactions may be failed;
     */
    async transferEVMToken(to, value, tokenAddress, params) {
        if (params?.units === 'ether')
            value = value * 10 ** 18;
        const payerPrivateKey = params?.payerPrivateKey || process.env.VLX_EVM_PRIVATE_KEY;
        if (!payerPrivateKey)
            throw new Error(`No payer key. Pass as param or env variable`);
        const web3 = new web3_2.default(new web3_2.default.providers.HttpProvider(this.rpcURL));
        let contract = new web3.eth.Contract(default_ERC_20_abi_json_1.default, tokenAddress);
        ...
        await web3.eth.sendSignedTransaction(signedTx.rawTransaction, function (error, hash) {
            ...
            logger_1.log.info(`Successfully sent ${value / 10 ** 18} tokens to ${to}. Tx hash: ${hash}`);
            txHash = hash;
        });
        return txHash;
    }
